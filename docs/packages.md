...
| `<!--pahlevan:sync version-->v3.1.0<!--/pahlevan:sync-->` | Immutable release tag (recommended for production) |
| `main` | Rolling tag for the default branch |
| `main-<sha>` | Per-commit build of the default branch, useful for bisecting |

```bash
docker pull ghcr.io/obsernetics/pahlevan:<!--pahlevan:sync version-->v3.1.0<!--/pahlevan:sync-->
```
...
docker inspect ghcr.io/obsernetics/pahlevan:<!--pahlevan:sync version-->v3.1.0<!--/pahlevan:sync-->

# Digest only (pin this in air-gapped or regulated environments)
docker inspect --format '{{index .RepoDigests 0}}' ghcr.io/obsernetics/pahlevan:<!--pahlevan:sync version-->v3.1.0<!--/pahlevan:sync-->

# Confirm the entrypoints exist without a shell in the image
docker run --rm --entrypoint /pahlevan-operator ghcr.io/obsernetics/pahlevan:<!--pahlevan:sync version-->v3.1.0<!--/pahlevan:sync--> --help
```
...
helm install pahlevan pahlevan/pahlevan-operator \
  ...
  --set image.tag=<!--pahlevan:sync version-->v3.1.0<!--/pahlevan:sync-->
```
...
```bash
kubectl apply -f https://github.com/obsernetics/pahlevan/releases/download/<!--pahlevan:sync version-->v3.1.0<!--/pahlevan:sync-->/install.yaml
```

Review before applying:

```bash
curl -sSL https://github.com/obsernetics/pahlevan/releases/download/<!--pahlevan:sync version-->v3.1.0<!--/pahlevan:sync-->/install.yaml | less
```
