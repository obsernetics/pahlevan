...
// The reference documents the storage version. v1alpha1 is still served for
// the deprecation window, but a reader opening this page is deciding what to
// write today, and pointing them at the version the project has committed to
// is the only answer that stays true.
const typesFile = "pkg/apis/policy/v1beta1/types.go"
...
func render(structs map[string]*structDoc) {
	...
	p("# API reference")
	p("")
	p("<!-- Generated by hack/apidocs from pkg/apis/policy/v1beta1/types.go. -->")
	...
	p("")
	p("The version described here is `policy.pahlevan.io/v1beta1`, which is what")
	p("the API server stores. `v1alpha1` is still served so existing objects and")
	p("clients keep working, and the API server converts between the two. The")
	p("v1alpha1 fields with no v1beta1 counterpart are listed, with the reason")
	p("each one is gone, in `IntentionallyDropped` in")
	p("`pkg/apis/policy/v1alpha1/conversion.go`.")
	p("")
