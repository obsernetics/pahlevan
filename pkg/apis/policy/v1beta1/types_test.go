			...
			LearningConfig: LearningConfig{
				...
				ExpectedBehavior: &ExpectedBehavior{
					...
					NetworkDestinations: []ExpectedDestination{{
						CIDR: "10.43.12.7/32", Port: 5432, Protocol: TransportProtocolTCP,
					}},
...
func fullContainerProfile() *ContainerProfile {
	...
	return &ContainerProfile{
		...
		Status: ContainerProfileStatus{
			...
			// exercised on each and so the two are visibly different lists.
			DeclaredFiles:               []string{"/var/lib/app/nightly.db (write)"},
