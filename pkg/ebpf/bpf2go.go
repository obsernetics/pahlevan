...
// loadMonitor is a generic function to load different types of eBPF monitors
func (m *EBPFProgramManager) loadMonitor(monitorType string) error {
	var spec *ebpf.CollectionSpec
	var err error

	// Load the appropriate monitor spec
	switch monitorType {
	case "syscall":
	...
	if err != nil {
		return fmt.Errorf("failed to load %s monitor spec: %v", monitorType, err)
	}

	// Create collection
	coll, err := ebpf.NewCollection(spec)
	if err != nil {
		return fmt.Errorf("failed to create %s collection: %v", monitorType, err)
	}

	// Store collection based on type
	switch monitorType {
	case "syscall":
	...
	// Store programs and maps with prefixes
	for name, prog := range coll.Programs {
		m.loadedPrograms[monitorType+"_"+name] = prog
	}
	for name, mapObj := range coll.Maps {
		m.loadedMaps[monitorType+"_"+name] = mapObj
	}

	// Attach programs based on type
	switch monitorType {
	case "syscall":
