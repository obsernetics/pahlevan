...
enforcement, and real-time monitoring of container behavior using eBPF technology.`,
	...
	flags.StringVar(&kubeconfig, "kubeconfig", "", "Path to kubeconfig file (uses in-cluster config if not set)")
	flags.StringVarP(&namespace, "namespace", "n", "", "Kubernetes namespace (uses current context namespace if not set)")
	...
	// Add Kubernetes configuration flags
