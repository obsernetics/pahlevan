...
def signals_falco(path, pod, cgroups):
    """Falco alerts, json_output=true, prefixed by the kubelet log timestamp."""
    out = []
    with open(path, "r", errors="replace") as fh:
        for raw in fh:
            raw = raw.strip()
            ...
            if brace < 0:
                continue
            k8s_ts = parse_rfc3339(raw[:brace]) if brace > 0 else None
            try:
                ev = json.loads(raw[brace:])
            except json.JSONDecodeError:
                continue
            if "rule" not in ev:
                continue  # falco's own startup/info logs
            ts = parse_rfc3339(ev.get("time")) or k8s_ts
            if ts is None:
                continue
            fields = ev.get("output_fields") or {}
            attributed = False
            if pod and fields.get("k8s.pod.name") == pod:
                attributed = True
            cname = fields.get("container.name")
            if cname and cname not in ("host", "<NA>"):
                # Falco names the container from the runtime; the target's
                # container is "app". Only used when pod name is unavailable.
                attributed = attributed or (not fields.get("k8s.pod.name") and cname == "app")
            out.append(
                {
                    "ts": ts,
                    "mech": "alert",
                    "denied": False,  # Falco is alert-only by design.
                    "label": "%s/%s" % (ev.get("priority", "?"), ev.get("rule", "?")),
                    "attributed": attributed,
                    "raw": raw[brace:],
                }
            )
...
def signals_tetragon(path, pod, cgroups):
    """Tetragon export-stdout JSON events."""
    out = []
    with open(path, "r", errors="replace") as fh:
        for raw in fh:
            raw = raw.strip()
            ...
            ts = parse_rfc3339(ev.get("time") or proc.get("start_time"))
            if ts is None:
                continue
            podinfo = proc.get("pod") or {}
            attributed = bool(pod) and podinfo.get("name") == pod
            binary = proc.get("binary", "")
            args = (proc.get("arguments") or "")[:60]
            action = body.get("action") or ""
            denied = action in ("KPROBE_ACTION_SIGKILL", "KPROBE_ACTION_OVERRIDE")
            label = "%s:%s" % (kind.replace("process_", ""), binary)
            if args:
                label += " " + args
            if action:
                label += " [" + action + "]"
...
PARSERS = {
    "pahlevan": signals_pahlevan,
    "falco": signals_falco,
    "tetragon": signals_tetragon,
}
